...
<head>
	<title>数据选择</title>
	<sys:jscss jscss="jquery,bootstrap,si,zTree"/>
	<script type="text/javascript">
		...
		var tree, setting = {view:{selectedMulti:false,dblClickExpand:false},
				...
				async:{enable:false,url:"${ctxA}/sys/user/treeData",autoParam:["id=menuId"]},
				data:{key:{name:"name"},simpleData:{enable:true,idKey: "id",pIdKey: "pId",rootPId: 0}},
				callback:{
					...
					onClick:function(event, treeId, treeNode){
						tree.expandNode(treeNode, null, false, false);
					},
		...
		$(function(){
			hjnUtils.ajax({  
		        type:'post',      
		        url:"${url}${fn:indexOf(url,'?')==-1?'?':'&'}&extId=${extId}&isAll=${isAll}&module=${module}&t="+new Date().getTime(),  
		        dataType:'json',  
		        success:function(data){
		        	if(data.code==1){
		        		// 初始化树结构
						tree = $.fn.zTree.init($("#tree"), setting, data.menus);
