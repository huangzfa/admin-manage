...
var consumerNo = GetQueryString("consumerNo")
...
$("#tpl_wrap").load("/dev/reportResources/tpl/tpl.html", function() {
    ...
    //社保+公积金数据+公司
    $.ajax({
        url: "/tpp/mxdata/fund/querySocialSecurityAndFundByConsumerNo.htm",
        method: 'get',
        dataType: "json",
        data: {
        ...
        success: function(result) {
            ...
            if (result) {
                //社保模板tab页。
                var source1 = $("#social-security-template").html(); //获取社保模板
                var template1 = Handlebars.compile(source1); //预编译模板
                var data = formatData(result.data);
                var returnHrml1 = template1(data); //生成html模板
                $("#social-security").html(returnHrml1)

                //公积金模板tab页。
                var source2 = $("#fund-template").html(); //获取公积金模板
                var template2 = Handlebars.compile(source2); //预编译模板
                var returnHrml2 = template2(data); //生成html模板
                $("#fund").html(returnHrml2)

                //人审模板页
                var source3 = $("#rensheng-template").html();//获取人审模板页
                var template3 = Handlebars.compile(source3); //预编译模板
                var returnHrml3 = template3(data); //生成html模板
                $("#rensheng").html(returnHrml3)

            } else if (result.data.mxdataSocialsecurityCode == 204) {
                $("#social-security").html("暂无数据")

            } else if(result.data.mxDataFundCode == 204){
                $("#fund").html("暂无数据")
            }
            else {
                $(".social-security").hide();
                $(".fund").hide();
            }
        }
    })
...
function formatData(data){
    ...
    for(var i = 0; i < data.listFund.length; i++){
        data.listFund[i].income = data.listFund[i].income/100;
    }


    if(typeof(data) != "null" && typeof(data) != 'undefined'){
        if(typeof(data.mxDataFund) != "null" && typeof(data.mxDataFund) != 'undefined'){

            // 将人民币 单位的 分转化为元

            if( typeof(data.mxDataFund) != 'undefined' && 'NORMAL' ==  data.mxDataFund.payStatus){
                data.mxDataFund.payStatus = '正常'
            }else if(data.mxDataFund.payStatus == ""){
                data.mxDataFund.payStatus = '不正常'
            }

            if(data.mxDataFund.realValid == 'Y'){
                data.mxDataFund.realValid = '一致'
            }else if(data.mxDataFund.realValid == 'N'){
                data.mxDataFund.realValid = '不一致'
            }

            //最近三个月有无断缴记录
            if(data.mxDataFund.nopayValid == 'Y'){
                data.mxDataFund.nopayValid = '有断缴';
            }else if(data.mxDataFund.nopayValid == 'N'){
                data.mxDataFund.nopayValid = '无断缴';
            }

            if(data.mxDataFund.payAddress == 'Y'){
                data.mxDataFund.payAddress = '是';
            }else if(data.mxDataFund.payAddress == 'N'){
                data.mxDataFund.payAddress = '否';
            }

            if(data.mxDataFund.isLoan == 'Y'){
                data.mxDataFund.isLoan = '有'
            }else if(data.mxDataFund.isLoan == 'N'){
                data.mxDataFund.isLoan = '无'
            }

            // mxDataFund.state}}(10，有效；20，无效；90,异常)
            if(data.mxDataFund.state = '10'){
                data.mxDataFund.state = '有效'
            }else if(data.mxDataFund.state = '20'){
                data.mxDataFund.state = '无效'
            }else if(data.mxDataFund.state = '90'){
                data.mxDataFund.state = '异常'
            }

        }

        if(typeof data.mxdataSocialsecurity !== "null" && typeof(data.mxdataSocialsecurity) != 'undefined'){

            //将人民币 分 转化为 元
            data.mxdataSocialsecurity.medicalBalance = data.mxdataSocialsecurity.medicalBalance/100;
            data.mxdataSocialsecurity.medicalAmountSum = data.mxdataSocialsecurity.medicalAmountSum/100;
            data.mxdataSocialsecurity.medicalMoneySum = data.mxdataSocialsecurity.medicalMoneySum/100;
            data.mxdataSocialsecurity.avgMedicalAmount = data.mxdataSocialsecurity.avgMedicalAmount/100;
            data.mxdataSocialsecurity.medicalBaseNumber = data.mxdataSocialsecurity.medicalBaseNumber/100;
            data.mxdataSocialsecurity.medicalInsuranceBalance = data.mxdataSocialsecurity.medicalInsuranceBalance/100;
            data.mxdataSocialsecurity.corporationPayment = data.mxdataSocialsecurity.corporationPayment/100;
            data.mxdataSocialsecurity.personalPayment = data.mxdataSocialsecurity.personalPayment/100;

            if(data.mxdataSocialsecurity.endowmentStatus == '0'){
                data.mxdataSocialsecurity.endowmentStatus = '正常'
            }else if(data.mxdataSocialsecurity.endowmentStatus != '1'){
                data.mxdataSocialsecurity.endowmentStatus = '停缴'
            }

            if(data.mxdataSocialsecurity.unemploymentStatus == '0'){
                data.mxdataSocialsecurity.unemploymentStatus = '正常'
            }else if(data.mxdataSocialsecurity.unemploymentStatus == '1'){
                data.mxdataSocialsecurity.unemploymentStatus = '停缴'
            }

            if(data.mxdataSocialsecurity.employmentStatus = '0'){
                data.mxdataSocialsecurity.employmentStatus = '正常';
            }else if(data.mxdataSocialsecurity.employmentStatus = '1'){
                data.mxdataSocialsecurity.employmentStatus = '停缴';
            }

            if(data.mxdataSocialsecurity.medicalStatus == '0'){
                data.mxdataSocialsecurity.medicalStatus = '正常';
            }else if(data.mxdataSocialsecurity.medicalStatus == '1'){
                data.mxdataSocialsecurity.medicalStatus = '停缴';
            }

            if(data.mxdataSocialsecurity.state == '10'){
                data.mxdataSocialsecurity.state = '有效'
            }else if(data.mxdataSocialsecurity.state == '20'){
                data.mxdataSocialsecurity.state = '无效'
            }
