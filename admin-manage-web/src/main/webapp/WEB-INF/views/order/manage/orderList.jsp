...
<head>
<title></title>
<sys:jscss jscss="jquery,webfont,bootstrap,si,css,easyui,select2,layer,dict,97Date" />
<!--  -->
<style type="text/css">
</style>
...
$(function(){
	...
	function getData(pageNum, pageSize){
		...
		data.inDate=$('#inDate').val().replace(/-/g,'');
		data.outDate=$('#outDate').val().replace(/-/g,'');
		data.orderStateZd=$('#orderStateZd').val();
		
		$('#tt').datagrid('loading');
		hjnUtils.ajax({  
	        type:'post',      
	        url:'${ctxA}/order/manage/orderListData',  
	        data:data,
	        dataType:'json',  
...
function orderIdformater(value,row,index){
	return '<a class="si-option-a noML" href="${ctxA}/order/manage/info?orderId='+value+'">'+value+'</a>';
}
</script>
</head>
<body>
...
<div class="breadcrumb form-search" style="margin-bottom:0;">
	<ul class="ul-form _clearfix">
		<li>
			...
			<label>入住时间：</label>
			<input id="inDate" placeholder="入住日期" class="input-small" type="text" onclick="WdatePicker({maxDate:'#F{$dp.$D(\'outDate\')}'})" />
			-
			<input id="outDate" placeholder="退房日期 " class="input-small" type="text" onclick="WdatePicker({minDate:'#F{$dp.$D(\'inDate\')}'})" />
		</li>
		<li>
			<label>订单状态：</label>
			<select id="orderStateZd" style="width:200px;">
				<option value="">全部</option>
				<c:forEach items="${cfg:getDictList('orderState')}" var="o">
...
<div class="si-warp" style="top:128px;">
