...
public class ExportUtil {
     ...
     * 
     */
    public static void responseSetProperties(String fileName, HttpServletResponse response,String gmtStart,String gmtEnd) throws UnsupportedEncodingException {
        // 设置文件后缀
    	 SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddHHmmss");
    	 String fn;
    	 if(StringUtils.isNotBlank(gmtStart) && StringUtils.isNotBlank(gmtEnd)){
    		 fn = fileName + gmtStart.split(" ")[0] + "-" + gmtEnd.split(" ")[0] + ".csv";
    	}else{
              fn = fileName + sdf.format(new Date()).toString() + ".csv";
    	}
        // 读取字符编码
        String utf = "UTF-8";
