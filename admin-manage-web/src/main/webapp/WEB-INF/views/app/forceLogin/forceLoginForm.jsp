...
<script>
    ...
    function getData(){
        ...
        hjnUtils.ajax({
            type:'post',
            url:'${ctxA}/app/forceLogin/forceLoginList',
            data:data,
            dataType:'json',
    ...
    function editState(id,status,switchId) {
        ...
        top.$.jBox.confirm(title,'系统提示',function(v,h,f){
            $('#tt').datagrid('loaded');
            if(v=='ok'){
                jQuery.post('${ctxA}/app/forceLogin/editState',{'isEnable':status,'switchId':switchId,'appId':$("#appId").val()},
                    function(data) {
                        if (data.code ==1) {
                            top.layer.alert("操作完成", {
                                icon: 6,
                                end: function(){
                                    window.location.href="${ctxA}/app/forceLogin/list?appId="+$("#appId").val();
                                }
                            });
                        } else {
