...
//				String className = GenerateUtils.toUpper(cn.substring(0));
				...
				String tplPath = "";
				if (db.equals("mysql")) {
					tplPath = StringUtils.replace(realProjectPath + "/duobei-console-tools/src/main/resources/generator/template/mysql", "/", separator);
				} else if (db.equals("oracle")) {
					//oracle 暂时不用
					//tplPath = StringUtils.replace(realProjectPath + "/collection-common/generate/template/oracle", "/", separator);
				}
...
//						new String[] { separator, separator });
				String dalPath = StringUtils.replaceEach(realProjectPath + "/duobei-console-core" + "/src/main/java/" + StringUtils.lowerCase(packageName), new String[] { "/", "." },new String[] { separator, separator });
				String bizPath = StringUtils.replaceEach(realProjectPath + "/duobei-console-core" + "/src/main/java/" + StringUtils.lowerCase(packageName), new String[] { "/", "." },new String[] { separator, separator });
				//String webPath = StringUtils.replaceEach(realProjectPath + "/duobei-console-web" + "/src/main/java/" + StringUtils.lowerCase(packageName) + "/web", new String[] { "/", "." },new String[] { separator, separator });
				
				// mybatis文件生成地址
				
				String batisPath = StringUtils.replaceEach(realProjectPath + "/duobei-console-core/src/main/resources/"+ StringUtils.lowerCase(packageName) + "/dao", new String[] { "/", "." }, new String[] { separator, separator });
				...
				if(needsMapper){
					...
					// 生成的文件地址
					String batisSql = batisPath + separator + model.get("ClassName") + "Dao.xml";
